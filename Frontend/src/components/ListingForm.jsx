  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        is_safe_deal_enabled: initialData.is_safe_deal_enabled !== undefined ? initialData.is_safe_deal_enabled : true,
      });
      setPriceType(initialData.priceType || 'price');
      setIsNegotiable(initialData.isNegotiable || false);
  ...
  const { data: config } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get('/products/filters-config/', {
        params: { category: formData.category_id }
      });
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-gray-900">
          {isEdit ? 'Редагувати оголошення' : 'Додати оголошення'}
        </h1>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form onSubmit={handleSubmitInternal} className="p-6 sm:p-8">
          ...
          <div className="mt-10 flex gap-4 justify-end border-t pt-6">
            ...
            >
              {isSubmitting ? 'Збереження...' : isEdit ? 'Зберегти зміни' : 'Опублікувати'}
            </button>
          </div>
        </form>
      </div>
